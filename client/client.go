...
var server = flag.String("server", "", "hostname or IP address of cryptux server")
...
func generateKey(password string) []byte {
	return argon2.IDKey([]byte(password), salt, 1, 64*1024, 4, 32) // 32 byte key
}
...
func decryptMessage(ciphertext, key [32]byte) (string, error) {
...
func main() {
	if *passcode == "!" {
		panic("Please provide a passcode (secret key) to encrypt your session.")
	}
